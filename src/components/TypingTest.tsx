...
import { CheckCircle, BarChart2, RefreshCcw, LogIn } from "lucide-react";
...
const TypingTest: React.FC = () => {
  ...
  const getTextPool = useCallback(() => {
    ...
    if (testOptions.punctuation && testOptions.numbers) {
      return getTextsByLength(
        shortPunctuationNumbersTexts,
        mediumPunctuationNumbersTexts,
        longPunctuationNumbersTexts
      );
    }

    if (testOptions.punctuation) {
      return getTextsByLength(
        shortPunctuationTexts,
        mediumPunctuationTexts,
        longPunctuationTexts
      );
    }
  ...
  const fetchAIText = useCallback(
    async (options?: TestOptions): Promise<string | null> => {
  ...
  const [typingData, setTypingData] = useState<{ time: number; wpm: number; accuracy: number }[]>([]);
  ...
  const saveTestResult = useCallback(async () => {
    ...
    try {
      const correctChars = typedText
        .split("")
        .filter((char, i) => text[i] === char).length;
      ...
      if (!response.ok) {
        ...
        toast({
          title: "Result saved",
          description:
            "Your typing test result has been saved to your profile!",
        });
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (typedText.length > 0 && startTime) {
      ...
      if (elapsedMinutes > 0) {
        ...
        const errPerMin = Math.round(
          (typedText.length - correctCount) / elapsedMinutes
        );
    ...
    if (
      (typedText.length === text.length || timeRemaining === 0) &&
      !testComplete
    ) {
      setTestComplete(true);
      setShowResults(true);
  ...
  const handleKeyDown = useCallback(
    (e: KeyboardEvent) => {
      ...
      if (testComplete && e.key !== "Tab" && !(e.key === "Enter" && tabPressed))
        return;
      ...
      if (e.key.length === 1) {
        setTypedText((prev) => {
          ...
          if (prev.length < text.length) {
            ...
            const isCorrect =
              nextIndex < text.length && e.key === text[nextIndex];
      ...
      if (lastChar) {
        setTypedText((prev) => {
          ...
          if (prev.length < text.length) {
            const nextIndex = prev.length;
            const isCorrect =
              nextIndex < text.length && lastChar === text[nextIndex];
  ...
  const renderText = () => {
    return text.split("").map((char, index) => {
      ...
      return (
        <span
          key={index}
          className="text-red-500 dark:text-red-400 bg-red-100 dark:bg-red-950/30"
        >
          {char}
        </span>
      );
  ...
  const renderResultsScreen = () => {
    ...
    const keystrokeEfficiency = Math.round(
    ...
    return (
      <Card className="w-full max-w-5xl mx-auto mt-6">
        <CardHeader>
          ...
          <CardDescription>
            Here&apos;s how you performed in your {testOptions.time} second{" "}
            {testOptions.mode} test
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="bg-muted/50 rounded-md p-4 text-center">
              ...
              <div className="text-lg sm:text-xl font-medium text-green-500">
                {correctChars}
              </div>
            </div>
            <div className="bg-muted/50 rounded-md p-4 text-center">
            ...
            <ul className="text-sm space-y-2">
              {peakWpm > wpm * 1.2 && (
                <li className="flex items-start">
                  ...
                  Your peak speed ({peakWpm} WPM) is significantly higher than
                  your average. With more practice, you could maintain this
                  speed longer.
                </li>
              )}
              {consistencyScore < 70 && (
                <li className="flex items-start">
                  ...
                  Your typing speed fluctuated during the test. Focus on
                  maintaining a steady rhythm to improve consistency.
                </li>
              )}
              {errorsPerMinute > 5 && (
                <li className="flex items-start">
                  <span className="mr-2 text-red-500">•</span>
                  You&apos;re making {errorsPerMinute} errors per minute.
                  Slowing down slightly might help improve accuracy.
                </li>
              )}
              {accuracy > 95 && wpm < 40 && (
                <li className="flex items-start">
                  <span className="mr-2 text-green-500">•</span>
                  Your accuracy is excellent! You might try increasing your
                  speed a bit without sacrificing precision.
                </li>
              )}
        ...
        <>
          ...
          >
            {isLoadingAI ? (
              ...
              <div className="text-lg sm:text-xl md:text-2xl lg:text-3xl leading-relaxed md:leading-loose tracking-wide font-mono">
                {renderText()}
              </div>
            )}
              ...
              <div className="flex items-center gap-2">
                {testComplete ? (
                  ...
                  <div className="text-xs sm:text-sm italic hidden sm:block">
                    Press{" "}
                    <kbd className="px-1 sm:px-2 py-0.5 sm:py-1 bg-muted rounded text-xs">
                      Tab
                    </kbd>{" "}
                    +{" "}
                    <kbd className="px-1 sm:px-2 py-0.5 sm:py-1 bg-muted rounded text-xs">
          ...
          {isSignedIn && (
            <div className="mt-2 text-xs text-center text-muted-foreground">
              Your results will be saved to your profile and may appear on the
              leaderboard.
            </div>
          )}
        </>
